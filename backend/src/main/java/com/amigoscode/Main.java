...
public class Main {
    ...
    CommandLineRunner runner(
        ...
        return args -> {
           createRundomCustomer(customerRepository, passwordEncoder);
    ...
    private static void testBucketUploadAndDownload(S3Service s3Service, S3Buckets s3Buckets) {
        s3Service.putObject(s3Buckets.getCustomer(),
                "foo",
                "Hello world assalam alykom".getBytes()
        );
    ...
    private static void createRundomCustomer(CustomerRepository customerRepository, PasswordEncoder passwordEncoder) {
        var faker = new Faker();
